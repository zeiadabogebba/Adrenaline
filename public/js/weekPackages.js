document.addEventListener('DOMContentLoaded', renderWeekPackages);

function renderWeekPackages() {
    const container = document.querySelector('.packages-container');
    if (!container) return;

    const weekPkgs = (window.SERVER_PACKAGES || []).filter(p => p.type === 'week' && p.status !== 'inactive');
    ...
    weekPkgs.forEach(pkg => {
        ...
        card.innerHTML = `
            <div class="trip-card__media media" style="background-image:url('${image}')">
                <span class="tag tag--crimson">${pkg.city || window.t('js.trip.egypt')}</span>
            </div>
            <div class="trip-card__body">
                <p class="label label--sm trip-card__kind">${window.t('js.trip.kindRehla')}</p>
